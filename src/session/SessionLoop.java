...
public class SessionLoop implements Runnable {
	...
	public void run() {
		...
		if (player1.getPlayer().getStatus().equals(Player.Status.IN_GAME))
			if (player2.getPlayer().getStatus().equals(Player.Status.IN_GAME))
				gameLoop();
		...
		if (player1.getPlayer().getStatus().equals(Player.Status.READY))
			if (player2.getPlayer().getStatus().equals(Player.Status.READY)) {
				...
				if (!inGame) {
					...
					player1.sendPacketToClient(Packets.P_YOURTURN);
					player2.sendPacketToClient(Packets.P_THEIRTURN);
					inGame = true;
				}
	...
	public void gameLoop() {
		...
		if (player1.getPlayer().isTurn()) {
			player1.sendPacketToClient(Packets.P_YOURTURN);
			player2.sendPacketToClient(Packets.P_THEIRTURN);
		} else {
			player1.sendPacketToClient(Packets.P_THEIRTURN);
			player2.sendPacketToClient(Packets.P_YOURTURN);
		}
	}
